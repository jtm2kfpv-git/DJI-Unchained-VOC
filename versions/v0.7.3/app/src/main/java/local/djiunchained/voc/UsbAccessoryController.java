...
final class UsbAccessoryController implements Closeable {
    ...
    synchronized boolean resendKeepalive() {
        if (session == null) {
            return false;
        }
        session.requestKeepalive();
        return true;
    }
    ...
    void recoverNow() {
        ...
        closeTransport();
        updateState(State.DISCONNECTED, 0, 0);
        listener.onStatus("Manual recovery: reopening USB accessory");
        connectFirstInternal(false);
    }

    void recoverStalledStream() {
        if (closed || !autoReconnect || manualDisconnect) {
            return;
        }
        cancelRetry();
        closeTransport();
        updateState(State.DISCONNECTED, 0, 0);
        listener.onStatus("Stream recovery: reopening USB accessory");
        connectFirstInternal(true);
    }
