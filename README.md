# DJI Unchained VOC 0.7.1 — Control and diagnostics iteration
...
> **Work in progress:** v0.7.1 is an active development tree. The v0.7.0 hardware-confirmed checkpoint remains preserved separately.

## What v0.7.1 adds

- Responsive bottom control tray sized for portrait and landscape operation.
- Large 48 dp minimum touch targets and a prominent record/stop control.
- Compact USB/video/resolution/FPS status strip.
- Separate source **Aspect**, display behavior, recording **Output**, and **30/50/60 fps** controls.
- Aspect/output controls lock while either recorder owns the pipeline.
- Retryable legacy Shorts errors and explicit original-stream recording labels.
- Collapsible, scrollable advanced section for connection details, recovery settings and diagnostics.
- Diagnostic schema 2 with a bounded 200-event timeline.
- Rolling 1/5/30-second FPS, current/average/minimum/maximum bitrate and longest frame gap.
- Decoder, parser, recovery, lifecycle, surface, orientation, refresh-rate, selected-storage, battery, charging, thermal, power-saving and memory information.
- Diagnostic export continues to exclude serials, accounts, location, network identifiers, document URIs and filesystem paths.
...
- The v0.6 MediaProjection-based Shorts recorder is retained only as a migration baseline while the direct video render/encode path is built.
